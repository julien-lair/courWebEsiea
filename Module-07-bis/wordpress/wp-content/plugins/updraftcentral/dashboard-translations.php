...
return array(
	...
	'general_js_comms_failure' => __('There was an error in contacting the remote site.', 'updraftcentral').' '.__("You should check that the remote site is online, is not firewalled, has remote control enabled, and that no security module is blocking the access.", 'updraftcentral').' '.__("Then, check the logs on the remote site and your browser's JavaScript console.", 'updraftcentral').' '.__('If none of that helps, then you should try re-adding the site with a fresh key.', 'updraftcentral'),
	'general_ajax_comms_failure' => __('There was an error in contacting either the dashboard server or the remote site.', 'updraftcentral').' '.__("You should check that the dashboard website is online, and that the remote site is online, is not firewalled, has remote control enabled, and that no security module is blocking the access.", 'updraftcentral').' '.__("Then, check the logs on the remote site and your browser's JavaScript console.", 'updraftcentral').' '.__('If none of that helps, then you should try re-adding the site with a fresh key.', 'updraftcentral'),
	'comms_failure_timeout' => __('There was an error in contacting the remote site.', 'updraftcentral').' '.__('The request did not complete in a reasonable amount of time.', 'updraftcentral').' '.__('You should check that the remote site is online, is not firewalled, has remote control enabled, and that no security module is blocking the access.', 'updraftcentral').' '.__('Then, check the logs on the remote site.', 'updraftcentral'),
	...
	'send_cors_headers_explanation' => __('CORS headers should normally be turned on, so that UpdraftCentral on the remote site sends these necessary headers.', 'updraftcentral').' '.__('You would only turn it off if you have set something else up to manually add them (which may be necessary for you to do if you are using HTTP authentication).', 'updraftcentral'),
	...
	'test_not_possible_in_current_mode' => __('These tests cannot currently be run.', 'updraftcentral').' '.__('You must first change and save the site configuration to take it out of the "Via mothership (dashboard webserver), encryption performed at the mothership" mode, in order to be able to perform the connection tests.', 'updraftcentral').' '.__('This is because in the current mode, some of the information for testing direct connections has not been shared with the browser, to enhance security.', 'updraftcentral'),
	...
	'localdev_can_work_better_with_https' => __('Browser security restrictions preventing https dashboards contacting non-https sites may be involved here.', 'updraftcentral').' '.__('You could try setting up https (i.e. SSL) for your site-local website, visiting its dashboard over https, and getting a new key for it.', 'updraftcentral'),
	...
	'site_unlicensed_message' => apply_filters('updraftcentral_site_unlicensed_message', __('You have more sites in your dashboard than licences.', 'updraftcentral').' '.__('As a result, you cannot perform actions on this site.', 'updraftcentral').' '.__('You will need to obtain more licences, or remove some sites.', 'updraftcentral')),
	...
	'rpcerrors' => array(
		...
		'rpc_fatal_error' => __('The remote site has encountered a fatal error while executing the requested command.', 'updraftcentral').' '.__('Remote response follows: %s', 'updraftcentral')
	),
	'unsupported_browser' => __('Your web browser lacks capabilities that UpdraftCentral requires, and thus is not supported.', 'updraftcentral').' '.__('Please update your browser to the latest version.', 'updraftcentral'),
	'search' => __('Search', 'updraftcentral'),
	'failed_credentials_heading' => __('Invalid Credentials', 'updraftcentral'),
	'failed_credentials' => __('The credentials you entered did not give the desired access.', 'updraftcentral').' '.__('Please check that your server is properly configured for access, and that you have entered the correct credentials.', 'updraftcentral'),
	...
	'process_completed' => __('The update process has completed.', 'updraftcentral').' '.__('Refreshing...', 'updraftcentral'),
	...
	'keyboard_shortcuts_message' => __("Most of these shortcuts operate on the currently selected site.", 'updraftcentral').' '.__("If no site is currently selected, then the first site in your list will be used.", 'updraftcentral').' '.__("You may change the default shortcuts, but be sure that your new shortcut does not conflict with your browser's own shortcut keys, otherwise, it won't work or may cause some unexpected behaviour.", 'updraftcentral').' '.__("Shortcuts are case-insensitive and only alphabetic (A-Z) characters are allowed.", 'updraftcentral'),
	...
	'keyboard_shortcut_exist' => __('The shortcut you wish to save has already been taken.', 'updraftcentral').' '.__('Please choose a different shortcut.', 'updraftcentral'),
	'all_sites_selected' => __('All sites selected.', 'updraftcentral').' '.__('Previous site selection cleared.', 'updraftcentral'),
	...
	'currently_processing' => __('An action is currently in progress.', 'updraftcentral').' '.__('Please wait for the process to complete before doing anything else.'),
	...
	'plugin_not_installed' => __('It appears that the %s plugin is not installed.', 'updraftcentral').' '.__('Press the "Install and Activate" button to download, install and activate the plugin on the remote website.', 'updraftcentral'),
	'plugin_not_activated' => __('It appears that the %s plugin is installed but not activated.', 'updraftcentral').' '.__('Press the "Activate" button to activate the plugin on the remote website.', 'updraftcentral'),
	...
	'timeout_subheading' => __('The maximum number of seconds until the request times out and the current process is stopped.', 'updraftcentral').' '.__('Adjust it to your specific needs and requirements.', 'updraftcentral'),
	...
	'export_message' => __('Without an encryption phrase, your exported data is in plain text.', 'updraftcentral').' '.__('Anyone who gets access to the unencrypted file would be able to control the exported sites.', 'updraftcentral').' '.__('Keep the phrase somewhere safe and do not lose it (there is no other way to decrypt the file).', 'updraftcentral'),
	...
	'import_successful' => __('Import process has succeeded.', 'updraftcentral').' '.__('Your UpdraftCentral dashboard will now be reloaded to refresh your sites list.', 'updraftcentral'),
	'encryption_phrase_required' => __('You have chosen encryption, but not entered an encryption phrase.', 'updraftcentral'),
	'keyboard_settings_message' => __('Shortcuts are here to help you navigate UpdraftCentral features more quickly.', 'updraftcentral').' '.__('If you wish to deactivate them, you can do so via the checkbox below.', 'updraftcentral'),
	...
	'yes_activate' => __('Replace existing with uploaded', 'updraftcentral'),
	'do_not_activate' => __('Cancel installation', 'updraftcentral'),
	...
	'load_subheading' => __('By default UpdraftCentral fills the entire content area when it loads.', 'updraftcentral').' '.__("If you don't wish to load UpdraftCentral that way, then un-tick the checkbox below.", 'updraftcentral').' '.__('You may need to reload the browser after saving for the new settings to take effect.', 'updraftcentral'),
	...
	'attempt_to_activate_failed' => __('The attempt to activate UpdraftPlus on "%s" has failed.', 'updraftcentral').' '.__('Please try again.', 'updraftcentral'),
);
